...
#[patch("/users/{id}", wrap = "RequireAuth")]
pub async fn patch(path: web::Path<(i32,)>, body: web::Json<UserUpdateInsert>) -> impl Responder {
    ...
    let result = diesel::update(users.filter(id.eq(eid)))
        .set((password_hash.eq(req.password_hash), login.eq(req.login)))
        .execute(connection);

    match result {
        Ok(_) => HttpResponse::Ok().finish(),
        Err(Error::DatabaseError(DatabaseErrorKind::UniqueViolation, _)) => {
            HttpResponse::Conflict().body("Login already used")
        }
...
#[delete("/users/{id}", wrap = "RequireAuth")]
pub async fn delete(path: web::Path<(i32,)>) -> impl Responder {
    use schema::users::dsl::*;
    let (eid,) = path.into_inner();
    let connection = &mut establish_connection();
    let result = diesel::delete(users.filter(id.eq(eid))).execute(connection);
