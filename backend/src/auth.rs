use std::{future::{self, Future, Ready}, pin::Pin};

use actix_web::{
     dev::{Service, ServiceRequest, ServiceResponse, Transform}, http::header::ContentType, HttpMessage, HttpResponse
};
...
impl actix_web::FromRequest for UserIdentity { // works
    type Error = ::actix_web::Error;
    type Future = std::future::Ready<Result<Self, Self::Error>>;
    fn from_request(req: &actix_web::HttpRequest, _: &mut actix_web::dev::Payload) -> Self::Future {
        std::future::ready(
            match <actix_web::HttpRequest as actix_web::HttpMessage>::extensions(req).get::<UserIdentity>() {
    ...
    S::Future: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let decode_result = decode::<Claims>(token, &DecodingKey::from_secret("secret".as_ref()), &Validation::default());
